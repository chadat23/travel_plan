...
def get_all(start_date: datetime = '%', entry_point: str = '%', end_date: datetime = '%', exit_point: str = '%',
    ...
    try:
        travels = session.query(Travel). \
            all()
            # filter(Travel.start_date.like(start_date)). \
            # filter(Travel.entry_point.like(entry_point)). \
            # filter(Travel.end_date.like(end_date)). \
            # filter(Travel.exit_point.like(exit_point)). \
            # all()
...
def add_travel(start_date: str, entry_point: str, end_date: str, exit_point: str,
               tracked: bool, plb: str, trip_leader_name: str,
               traveler_units: List[TravelUserUnit], car_name: str, car_location: str
               ):
        ...
        travel = Travel(start_date, entry_point_id, end_date, exit_point_id,
                        tracked, plb, trip_leader_id, car_id, car_location)

        session.add(travel)

        for p_u in traveler_units:
